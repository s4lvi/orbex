...
2. the player has a 'ship' which is  
   what transports them to the planets, and starts only able to reach the first planet, but they can upgrade the  
   ship using basic resources to reach the second planet, and use exotic resources from the second planet to reach  
   the third.
...
- Portrait (720x1280) for mobile browsers
...
- Completing a landing zone awards the resources shown in the preview to the initial amount you take into new landings
- Small bonus resources between waves

**Spending Resources:**

- Building turrets, adding upgrades, or traps costs basic and/or exotic resources
- Upgrading your ship to travel to new planets costs exotic resources.
...
When a player selects a planet, display a map of that planet with resource nodes randomly dispersed. The landing site will encompass a radius, and a successful landing will secure all the resources in that radius. The more resources within the radius (along with the planet modifiers) set the difficulty target of that drop.

**Map Generation based on landing zone difficulty:**

- Number of paths: 1-4 (more paths = higher difficulty)
- Path complexity: Random waypoints between entry and exit
- Paths enter from edges, base at the center of the map.
